...
if (iconMenu != null) {
	let delay = 500;
	let menuBody = document.querySelector(".header__box");
	let menuBodyColor = document.querySelector(".backcolor");
	iconMenu.addEventListener("click", function (e) {
		if (unlock) {
			body_lock(delay);
			iconMenu.classList.toggle("_active");
			menuBody.classList.toggle("_active");
			if(menuBodyColor != null){
				menuBodyColor.classList.toggle("_active");
			}
		}
	});
};
function menu_close() {
	...
	if(menuBodyColor != null){
		menuBodyColor.classList.remove("_active");
	}
}
//=================
//BodyLock
...
function popup_open(item, video = '') {
	...
	if (curent_popup && unlock) {
		...
		if (!document.querySelector('.header__box._active')) {
			body_lock_add(500);
		}
...
function popup_close(item, bodyUnlock = true) {
	if (unlock) {
		...
		if (!document.querySelector('.header__box._active') && bodyUnlock) {
